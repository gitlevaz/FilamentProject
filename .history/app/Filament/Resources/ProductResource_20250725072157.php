...
TextInput::make('address')
    ...
    ->suffixAction(
        FormAction::make('validate')
            ...
            ->action(function ($state, $livewire) {
                try {
                    // ✅ Extract street details from full address (e.g., "20 King Street")
                    $fullAddress = trim($state);
                    ...
                    $payload = [
                        ...
                        'suburb' => 'Melbourne', // You can make this a form field if needed
                        'postcode' => '3000',
                        'state' => 'VIC',
                    ];

                    logger()->info('Find Address Payload:', $payload);
                    ...
                    logger()->info('Find Address Response:', $findResponse->json());
                    ...
                    $directoryId = $findResponse->json()[0]['DirectoryIdentifier'] ?? null;

                    if (!$directoryId) {
                        Notification::make()
                            ->title("No Directory ID returned")
                            ->danger()
                            ->send();
                        return;
                    }
                    ...
                    logger()->info('Qualify Response:', $qualifyResponse->json());
                    ...
                    if ($qualifyResponse->ok() && $qualifyResponse->json('result') === 'Success') {
                        ...
                        Notification::make()
                            ->title("Qualification failed for: {$directoryId}")
                            ->danger()
                            ->send();
                    }
                } catch (\Exception $e) {
